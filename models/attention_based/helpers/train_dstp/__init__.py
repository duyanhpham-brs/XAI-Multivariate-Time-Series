...
def dstp_rnn(
    ...
    if save_path:
        ...
        print("Resuming training from " + os.path.join(save_path, "dstp_encoder.pth"))
        print("Resuming training from " + os.path.join(save_path, "dstp_decoder.pth"))
        encoder.load_state_dict(
            torch.load(os.path.join(save_path, "dstp_encoder.pth"), map_location=device)
        )
        decoder.load_state_dict(
            torch.load(os.path.join(save_path, "dstp_decoder.pth"), map_location=device)
        )
...
def train(
    ...
    print(os.path.join(dir_path, "checkpoint", "dstp_encoder.pth"))
    print(os.path.join(dir_path, "checkpoint", "dstp_decoder.pth"))
    torch.save(
        net.encoder.state_dict(), os.path.join(dir_path, "checkpoint", "dstp_encoder.pth")
    )
    torch.save(
        net.decoder.state_dict(), os.path.join(dir_path, "checkpoint", "dstp_decoder.pth")
    )
