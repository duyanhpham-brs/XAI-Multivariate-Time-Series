...
def test_load_function(set_up):
    ...
    assert feature_maps.cam(inputs, True).shape == expected.size()
...
def test_show_function_with_upsampling(set_up):
    ...
    feature_maps = set_up
    feature_maps.load(None, None, None, None)
    outputs = feature_maps.show(inputs, True, None)
...
def test_show_function_without_upsampling(set_up):
    ...
    feature_maps = set_up
    feature_maps.load(None, None, None, None)
    outputs = feature_maps.show(inputs, True, None, None, False)
