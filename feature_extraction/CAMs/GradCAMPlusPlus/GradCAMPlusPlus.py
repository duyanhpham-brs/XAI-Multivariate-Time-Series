...
class GradCAMPlusPlus(GradCAM):
    ...
    def map_gradients(self):
        if len(self.grads_val.shape) == 4:
            weights = np.sum(F.relu(self.grads_val).numpy()*self.alphas, axis=(2, 3))[0, :]
    ...
    def __call__(self, input_features, index=None):
        one_hot, self.grads_val, self.target = self.calculate_gradients(input_features, index)
        ...
        self.alphas = self.extract_higher_level_gradient(global_sum, second_derivative, third_derivative)
